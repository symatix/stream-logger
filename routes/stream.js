...
module.exports = (app) => {
    ...
    app.get('/api/streams/scan', async (req, res) => {
        ...
        getDataFromAllStations(stations, []).then(data => {
    ...
    app.get('/api/streams', async (req, res) => {
        const streams = await Streams.find({}, null, {sort: {station: 1}});
        res.send(streams);
    })
    app.get('/api/streams/:station', async (req, res) => {
        const { station } = req.params;
        const streams = await Streams.find({ station }, null, {sort: {time: 1}});
        res.send(streams);
    })
}